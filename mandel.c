/*******************************************************************
* Filename: mandel.c
* Description: generates fractal image using multi processes & threads
...
int main( int argc, char *argv[] ) {
	...
	int    max = 1000;
    int    num_threads = 1; // Defaults to 1 thread
	...
	while((c = getopt(argc,argv,"x:y:s:W:H:m:o:h:t:"))!=-1) {
		switch(c) {
			...
			case 'o':
				outfile = optarg;
				break;
            case 't': // Parse number of threads
...
// Worker thread function to compute a specific region of the image.
void* compute_region(void* arg) {
    ...
    int start_y = data->thread_id * height / data->total_threads;
    int end_y = (data->thread_id + 1) * height / data->total_threads;
...
void compute_image(imgRawImage* img, double xmin, double xmax, double ymin, double ymax, int max, int num_threads) {
    ...
    for (int t = 0; t < num_threads; t++) {
        ...
        if (pthread_create(&threads[t], NULL, compute_region, (void*)&thread_data[t]) != 0) {
